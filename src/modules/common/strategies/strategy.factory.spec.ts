...
describe('Strategy Factory', () => {
  ...
  it('should return flat', () => {
    ...
    expect(
      StrategyFactory.createStrategy(strategyName, gbm, flat, noiseWave),
    ).toBeInstanceOf(FlatMarketSimulationStrategy);
    expect(
      StrategyFactory.createStrategy(strategyName, gbm, flat, noiseWave),
    ).toBe(flat);
  });
  it('should return gbm', () => {
    ...
    expect(
      StrategyFactory.createStrategy(strategyName, gbm, flat, noiseWave),
    ).toBeInstanceOf(GBMMarketSimulationStrategy);
    expect(
      StrategyFactory.createStrategy(strategyName, gbm, flat, noiseWave),
    ).toBe(gbm);
  });
  it('should return noise wave', () => {
    ...
    expect(
      StrategyFactory.createStrategy(strategyName, gbm, flat, noiseWave),
    ).toBeInstanceOf(NoiseWaveSimulationStrategy);
    expect(
      StrategyFactory.createStrategy(strategyName, gbm, flat, noiseWave),
    ).toBe(noiseWave);
  });
